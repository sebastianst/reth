...
// branch targets finalized, falling back to head when the CL has no finalized hash yet.
...
fn test_backfill_target_hash_eth_returns_finalized() {
    ...
    assert_eq!(test_harness.tree.backfill_target_hash(), Some(finalized));
}

#[test]
fn test_backfill_target_hash_eth_zero_finalized_falls_back_to_head() {
    let mut test_harness = TestHarness::new(MAINNET.clone());
    let head = B256::from([0xAA; 32]);
    test_harness.tree.state.forkchoice_state_tracker.set_latest(
        ForkchoiceState {
    ...
    // Optimistic sync: CL has no finalized hash yet; backfill targets head.
    assert_eq!(test_harness.tree.backfill_target_hash(), Some(head));
}
...
#[test]
fn test_backfill_target_hash_no_state_returns_none() {
    let test_harness = TestHarness::new(MAINNET.clone());
    assert_eq!(test_harness.tree.backfill_target_hash(), None);
}

#[test]
fn test_backfill_target_hash_all_zero_returns_none() {
    let mut test_harness = TestHarness::new(MAINNET.clone());
    test_harness.tree.state.forkchoice_state_tracker.set_latest(
        ForkchoiceState {
            head_block_hash: B256::ZERO,
